...
RUN_AS=root
BHOMA_PID_FILE=/var/run/${NAME}_patient_formlistener.pid

test -x $DAEMON || exit 0

do_start() {
    echo -n "Starting bhoma patient formlistener"
    start-stop-daemon -d $APP_PATH -c $RUN_AS --start --background --pidfile $BHOMA_PID_FILE  --make-pidfile --exec $DAEMON -- $MANAGE_PY_LOCATION patient_formlistener
    echo "patient formlistener started"
}

hard_stop_bhoma() {
    for i in `ps aux | grep -i "patient_formlistener" | grep -v grep | awk '{print $2}' ` ; do
        kill -9 $i
    done
    rm $BHOMA_PID_FILE 2>/dev/null
    echo "Hard stopped patient formlistener"
}
...
do_stop() {
    echo -n "Stopping bhoma patient formlistener.. "
