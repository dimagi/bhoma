...
def poor_mans_xml(template, format_dict):
    # the escaping in this module is pretty brittle. it might be time
    # to refactor and use some real XML tools for this job.
    cleaned_args = dict((key, escape("%s" % val)) for key, val in format_dict.items())
    return template % cleaned_args
...
def get_response(xform_doc, forms_today=1, total_forms=1):
    return poor_mans_xml(RESPONSE_TEMPLATE, 
                         {"status_code": 2000,
                          "submit_code": 200,
                          "id": xform_doc.get_id,
...
def get_registration_xml(chw):
    ...
    if referral_record and "value" in referral_record:
        ...
        referral_count = 0
    return poor_mans_xml(REGISTRATION_TEMPLATE,
...
UPDATE_BLOCK = \
"""
    <update>%(update_base_data)s%(update_custom_data)s
    </update>"""
...
def get_case_xml(phone_case, create=True):
    ...
    base_data = poor_mans_xml(BASE_DATA,
                              {"case_type_id": phone_case.case_type_id,
    ...
    update_block = UPDATE_BLOCK % {"update_base_data": update_base_data,
                                   "update_custom_data": custom_update_data}
    return CASE_TEMPLATE % {"case_id": escape(phone_case.case_id),
                            "date_modified": escape(date_to_xml_string(phone_case.date_modified)),
