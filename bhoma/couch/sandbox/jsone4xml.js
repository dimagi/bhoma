...
var e4x2json = function(xml) {
        toReturn = {};
        this.log_func("e4x2json!");
        allElems = xml.*;
        this.log_func("type: " + allElems.constructor.name);
        this.log_func("all elems: " + allElems);
        for (elem in allElems) {
            
            this.log_func("elem: " + allElems[elem] + ", type(key): " + elem.constructor.name + ", type(val):" + allElems[elem].constructor.name);
            if (elem instanceof XMLList) {
                this.log_func("it is a list!");
            }
        }
        return toReturn;
}
